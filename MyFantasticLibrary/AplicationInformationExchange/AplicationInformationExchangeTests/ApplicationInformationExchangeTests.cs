...
namespace AplicationInformationExchangeTests
{
    ...
    {
        private Message Send(Message message, int port, int bufferSize = 10240)
        {
            Sender sender = new Sender("127.0.0.1", 4000);
            return sender.Send(message);
        }
        private Message ReceiveOne(int port, Func<Message, Message> okResult, Func<bool> end, int bufferSize = 10240)
        {
            Receiver receiver = new Receiver("127.0.0.1", 4000, okResult, end, bufferSize);
        ...
        public void TestSendTwoMessages()
        {
            ...
            Task receiverTask = Task.Run(() =>
            {
               ReceiveMultiple(4000, okResult, end);
            });
            Thread.Sleep(100);
        ...
        [TestMethod]
        public void TestSendMultipartPage()
        {
            int index = 0;
            Func<Message, Message> okResult = (m) =>
            {
            ...
            Task receiverTask = Task.Run(() =>
            {
                result = ReceiveOne(4000, okResult, end, 2);
            });
            Thread.Sleep(100);
